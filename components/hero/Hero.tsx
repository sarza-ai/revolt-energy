...
export function Hero() {
  return (
    ...
    >
      ...
      */}
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-1 flex-col items-center px-4 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1 text-center sm:px-6 sm:pb-14 sm:pt-4 max-lg:min-h-0 max-lg:justify-center max-lg:gap-2 lg:pointer-events-none lg:flex-none lg:gap-0 lg:pb-16 lg:pt-0">
        <motion.div
        ...
        >
          <div className="mb-2 inline-flex items-center gap-1.5 rounded-full border border-emerald-500/35 bg-black/55 px-3 py-1 text-[0.95rem] font-medium text-white backdrop-blur-sm sm:mb-5 sm:gap-2 sm:px-5 sm:py-2 sm:text-sm">
            <Leaf className="h-4 w-4 shrink-0 text-emerald-400 sm:h-4 sm:w-4" />
            ReVolt Energy
          </div>

          {/* Bumped mobile headline; still clamped to one-screen layout */}
          <h1 className="text-[clamp(2.2rem,9.8vw,3.15rem)] font-bold leading-[1.04] tracking-[-0.035em] sm:mb-5 sm:text-5xl md:text-6xl lg:text-7xl">
        ...
        <motion.div
          ...
          className="pointer-events-auto relative my-0.5 flex min-h-0 w-full shrink items-center justify-center sm:my-2 lg:hidden"
        >
        ...
        <motion.div
          ...
          className="w-full shrink-0"
        >
          <p className="mx-auto mb-3 max-w-xl text-[1.05rem] leading-snug text-gray-300 sm:mb-8 sm:max-w-2xl sm:text-lg sm:leading-relaxed md:text-xl">
          ...
          <div className="pointer-events-auto mx-auto grid w-full grid-cols-2 gap-2.5 sm:flex sm:max-w-none sm:flex-row sm:justify-center sm:gap-4">
            <a
              href="#solutions"
              className="inline-flex min-h-11 items-center justify-center rounded-xl bg-emerald-600 px-2 py-2.5 text-center text-[0.9rem] font-semibold leading-tight text-white shadow-[0_0_24px_rgba(16,185,129,0.3)] transition active:scale-[0.98] hover:bg-emerald-500 sm:min-h-12 sm:rounded-2xl sm:px-10 sm:py-3.5 sm:text-lg"
            >
              See the Technology
            </a>
            <a
              href="/contact"
              className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/55 px-2 py-2.5 text-center text-[0.9rem] font-semibold leading-tight text-white transition active:scale-[0.98] hover:bg-white/10 sm:min-h-12 sm:rounded-2xl sm:px-10 sm:py-3.5 sm:text-lg"
            >
              Join the ReVolt
            </a>
          </div>
